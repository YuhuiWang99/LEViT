...
def make_list(exdir):
    ...
    for _, _, files in os.walk(train_dir, topdown=False):
        for pname in files:
            if '.jpg' in pname or '.png' in pname :
...
def gen_lmdb(exdir, name):
    lmdb_path = osp.join(exdir, "{}.lmdb".format(name))
...
def main():
    ...
    if os.path.exists(fpath):
        if not osp.isdir(exdir):
            extract(fpath, root)
            make_list(osp.join(exdir, 'detected'))
            make_list(osp.join(exdir, 'labeled'))
        else:
            make_list(osp.join(exdir, 'detected'))
            make_list(osp.join(exdir, 'labeled'))
    else:
    ...
    gen_lmdb(osp.join(exdir, 'detected'), "npdetected")
    gen_lmdb(osp.join(exdir, 'labeled'), "nplabeled")
