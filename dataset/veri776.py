...
def make_list(exdir):
    train_dir = osp.join(exdir, "image_train")
    train_list = {}
    for _, _, files in os.walk(train_dir, topdown=False):
        for pname in files:
            if '.jpg' in pname:
                pname_split = pname.split('_')
                pid = int(pname_split[0])
                pcam = int(pname_split[1][1:])
...
def gen_lmdb(exdir):
    lmdb_path = osp.join(exdir, "veri776.lmdb")
    ...
    for idx, line in enumerate(item_list):
        ...
        pname = osp.join(exdir, "image_train", pname)
        pid = int(pid)
        pcam = int(pcam)
