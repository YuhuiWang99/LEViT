...
from .layer import BatchErasing

from .LEViT import LEViT


class DBN(nn.Module):
    def __init__(self, num_classes=751, num_parts=[1,2], std=0.1, net="small"):
        super(DBN, self).__init__()
        self.num_parts = num_parts
        if self.training:
            self.batch_erasing = BatchErasing(smax=0.4, type="image")
        ...
        if net == "small":
            ...
            feat_num = 384
        if net == "large":
            base = LEViT(num_classes=1000, stem=16, embed_dim=192, mlp_ratio=1., layers=[4,7,4], num_heads=[2,4,8], split_size=[8,8,8], drop_path=0.15, use_vit=[1,1,0])
