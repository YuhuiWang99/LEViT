...
if __name__ == '__main__':
    ...
    lines = sio.loadmat(osp.join(data_dir, "cars_annos.mat"))["annotations"][0]

    test_set = []
    test_label = []

    for idx in range(len(lines)):
        name, bbox_x1, bbox_y1, bbox_x2, bbox_y2, label, test = lines[idx]
        name = name.squeeze().tolist()
        label = label.squeeze().tolist()
        if label > 98:
            test_set.append([osp.join(data_dir, name), label])
    ...
    # net = DBN(num_classes=98, num_parts=[1,2], net="small")
    net = DBN(num_classes=98, num_parts=[1,2], net="large")
    ...
    cluster_labels = get_cluster_labels(test_feature, 98)
